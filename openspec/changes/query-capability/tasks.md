...
- [ ] 1.2 合成器生成 HAVING（agg_expr + op + value；无 GROUP BY 也支持）；MVP 限 aggregate/trend
- [ ] 1.3 单测：指标过滤 HAVING、维度过滤 WHERE、混合（WHERE+HAVING）、非聚合误用降级

## 2. 合成器：跨源同粒度多指标（子查询 JOIN）

- [ ] 2.1 多指标跨 sourceTable → 各指标子查询（SELECT 维度键 + agg AS code FROM source GROUP BY 维度键）+ 按维度键 JOIN
- [ ] 2.2 约束：共享 dims/time/filters + 粒度对齐；不对齐 → SynthesisError
- [ ] 2.3 单测：n02 子查询 JOIN SQL 形态、异粒度降级、同源多指标行为不变
