...
#### Scenario: 跨源同粒度多指标（子查询 JOIN）
- **WHEN** 多指标来自不同 sourceTable（如 play_detail + user_behavior_fact）且共享同一组维度键/时间/过滤
- **THEN** 合成子查询 JOIN SQL（各指标独立聚合子查询，按维度键对齐），在真实数据库可解析执行
