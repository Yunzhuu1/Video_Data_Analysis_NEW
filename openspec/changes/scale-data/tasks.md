...
- [ ] 2.1 metric_catalog.json 新增 8 指标（comment_rate/like_rate/share_rate/avg_completion_ratio/creator_revenue/video_revenue/active_creator_count/daily_active_users）
- [ ] 2.2 aliases.yaml 补新指标用户说法映射
...
- [ ] 4.1 --memory off 新基线评测（N=~70）：既有 45 例子集 L1-L4/R1 零回退 + 新增 ~25 例独立报告
- [ ] 4.2 R1 全量（新旧用例）验证；合成器对新表/新指标查询不报未捕获错误（已知边界标 fallback）
- [ ] 4.3 metrics-report 更新（新基线 + 新指标/表）+ 开发日志
