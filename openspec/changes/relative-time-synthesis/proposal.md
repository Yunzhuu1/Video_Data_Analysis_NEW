...
- **R1 闭环验证**：修复后 c03 等 relative 用例合成 SQL 含 `WHERE date BETWEEN ...`，R1 扩展到 relative 用例（取真值并断言）。
...
- `agent-engine/app/eval/runner.py`：R1 扩展到 relative 用例（取真值）；mock anchor 注入。
