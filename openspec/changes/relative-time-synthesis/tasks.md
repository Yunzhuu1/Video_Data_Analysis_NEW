...
- [ ] 1.1 `time_expand.py`：relative {amount, unit} + anchor_date → absolute（含端点：最近N天 = 末日往前 N-1 天）；unit day/week（month 近似 30 天标注）
- [ ] 1.2 单测：7天/30天/一周/昨天到今天、含端点边界、unit 换算
...
- [ ] 4.2 --memory off N=45 回归零回退（L1-L4 不变）
- [ ] 4.3 R1 评测：relative 用例全绿（修复闭环证据）
- [ ] 4.4 metrics-report 更新（relative 修复 + R1 扩展）；开发日志追加
