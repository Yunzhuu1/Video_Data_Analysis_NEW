...
- Enumerator 生成 Candidate 时写入实际 `snapshotFingerprint` 并纳入 planId；Validator PASS 返回该 fingerprint，Graph 冻结为 `validated_snapshot_fingerprint`；Compiler 要求私有冻结副本、validated fingerprint 与 plan fingerprint 三者完全一致，防止用自洽 S1 替换 Validator 验证过的 S0。
...
- 兼容性：SQL 与主链路业务行为不变；planId 因首次纳入 snapshotFingerprint 发生一次预期版本变化，之后对同一 snapshot 保持确定性。planId 仅在单次 run 内使用、无持久化/外部消费者；非法 snapshot 不再进入 plan compiler。
