...
- **新增 `real-session` 评测协议**（真实路径记忆评测）：namespace=`real-<ts>`，按多轮会话执行（首问沉淀 → 二问命中 → 近似问注入），验证跨请求命中与持久化（进程重启后命中仍在）。
...
- **新增真实路径指标**：真实命中率、跨请求持久化命中数、真实直通 token 省、真实同问同答一致率，写入 metrics-report「真实路径基线」章节。
