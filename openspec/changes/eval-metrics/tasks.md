...
- [ ] 3.1 直通收益实验（hit 波段）：重复对第一遍 vs 第二遍延迟/用例总 token 差（≈ 解析阶段消除）
- [ ] 3.2 注入收益实验（仅 inject 波段子集）：无记忆 vs 有记忆 L1 成功率 + inject 命中率；miss 子集单独报告；实验前检查：沉淀后算全量运行时 band，inject 子集 < 8 条（阈值可配）→ 警告 + 报告显式标注「样本不足，结论仅方向性」（不阻断但可见）
- [ ] 3.3 冷热启动实验（检索侧）：空记忆 vs seed 预置记忆成功率/延迟（区别于全链路注入实验）
