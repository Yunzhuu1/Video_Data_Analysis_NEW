...
- [ ] 5.1 runner --memory off|on（默认 off）；**--memory on 用独立记忆库（:memory:/临时文件）**；conftest :memory: 隔离
- [ ] 5.2 重复问题对用例（一致率 100%，口径=两遍都成功）+ 相似反例用例（点赞量不命中播放量）+ memory_hit_rate/inject_rate 指标
...
- [ ] 5.3 Python pytest + ruff 全绿
- [ ] 5.4 真实评测：--memory off 全量回归（L1-L4 100% 不回退）→ --memory on 重复对/反例（一致率 100%、反例不命中、命中率/延迟/token 对比）
